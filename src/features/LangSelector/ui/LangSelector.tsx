...
export const LangSelector: FC<LangSelectorProps> = memo<LangSelectorProps>(
    (props: LangSelectorProps) => {
        const { i18n } = useTranslation();

        const onSelect: MouseEventHandler<HTMLButtonElement> = (
            event: LangSelectorEvent,
        ): void => {
        ...
        return (
            <div
                className={classNames(cls.LangSelector, {}, [props.className])}>
                <Button onClick={onSelect} theme={ButtonTheme.CLEAR_INVERTED}>
                    <img
                        className={classNames(
                            cls.img,
                            { [cls.active_lang]: i18n.language === 'ru' },
                            [],
                        )}
                        src={RusFlag}
                        alt={'ru'}
                ...
                <Button onClick={onSelect} theme={ButtonTheme.CLEAR_INVERTED}>
                    <img
                        className={classNames(
                            cls.img,
                            { [cls.active_lang]: i18n.language === 'en' },
                            [],
                        )}
                        src={GBFlag}
                        alt={'en'}
